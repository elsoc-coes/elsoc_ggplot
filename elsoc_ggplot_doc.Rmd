...
library(pacman)
pacman::p_load(car,dplyr,panelr,stringr,tidyverse,ggplot2,survey)
...
- `c05_02`: nivel de confianza con los partídos políticos de la persona encuestada
...
elsoc_wide <- elsoc_wide_2016_2019 %>% dplyr::filter(tipo_atricion==1 & tipo_caso !=2) 
...
```{r Recode de var "tipo de ciudad"}
...
```{r Recode de var "cambiar_consti"}
...
#Crear tabla que agrupa las frecuencias por cada respuesta
a <- data.frame((svytable(~cambiar_consti + ola, elsoc_diseno, round = F))) 
#Crear tabla que agrupa las frecuencias dentro de cada año
b <- a %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
#Quitar variables repetidas
b$ola <- NULL
#Unir las dos tablas creadas anteriormente
...
En las siguientes secciones se elaboran algunos de los gráficos más comunes para presentar data cualitativa y cuantitativa para encuestas longitudinales. Se recomienda revisar literatura relevante para la visualización de datos (disponible en [sección 3](#literatura)) para representar graficamente la información y datos en forma óptima.
...
a <- data.frame((svytable(~conf_presi_rec + ola, elsoc_diseno, round = F)))
b <- a %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
b$ola <- NULL
datos.grafico<- cbind(a,b)
```
...
```{r}
a1 <- data.frame(svytable(~conf_presi_rec + ola + edad, elsoc_diseno, round = T))
a2 <- a1 %>% group_by(edad, ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$edad <- NULL
a2$ola <- NULL
...
```{r tres_var_ola}
a <- data.frame(svytable(~conf_presi_rec + ola, elsoc_diseno, round = T))
a1 <- a %>% group_by(ola) %>% transmute(p_presi=Freq/sum(Freq))
b <- data.frame(svytable(~conf_part_rec + ola, elsoc_diseno, round = T))
b1 <- b %>% group_by(ola) %>% transmute(p_part=Freq/sum(Freq))
c <- data.frame(svytable(~conf_carb_rec + ola, elsoc_diseno, round = T))
...
datos.grafico3 <- data.frame(cbind(elsoc_long$idencuesta, elsoc_long$ola,elsoc_long$estatus, 
...
```{r recodificar periodos de cambio}
...
```{r}
#Paso 1
a1 <- data.frame(svytable(~idcoal + ola + idencuesta, elsoc_diseno, round = T))
a1 <- a1 %>% dplyr::filter(Freq==1)
a2 <- a1 %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$ola <- NULL
datos.d.1<- cbind(a1,a2)

#Paso 2
b1 <- data.frame(svytable(~idcoal + ola, elsoc_diseno, round = T))
b2 <- b1 %>% 
  group_by(ola) %>% 
...
```{r}
#Paso 1
a1 <- data.frame(svytable(~idcoal + ola + idencuesta + socup + sexo, elsoc_diseno, round = T))
a1 <- a1 %>% dplyr::filter(Freq==1)
a2 <- a1 %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$ola <- NULL
datos.d.3<- cbind(a1,a2)

#Paso 1.1: crear un subset sólo para los años 2017 y 2019
subset.d.3 <- droplevels(subset(datos.d.3, datos.d.3$ola == '2017' | datos.d.3$ola == '2019'))

#Paso 2
b1 <- data.frame(svytable(~idcoal + ola + socup + sexo, elsoc_diseno, round = T))
b2 <- b1 %>% 
  group_by(ola) %>% 
  transmute(porcentaje=Freq/sum(Freq)) %>% 
  mutate(idencuesta = 1)
b2$ola <- NULL
etiquetas.d.3<- cbind(b1,b2)
...
```{r}
#Paso 1
a1 <- data.frame(svytable(~conf_presi + ola, elsoc_diseno, round = T))
a2 <- a1 %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$ola <- NULL
datos.e.1<- cbind(a1,a2)
```
...
c <- data.frame(svytable(~conf_carb_rec + ola, elsoc_diseno, round = T))
c1 <- c %>% group_by(ola) %>% transmute(p_carb=Freq/sum(Freq))
datos.grafico.e.2<- cbind(a,a1,b,b1,c,c1)
...
```{r}
#Paso 1: Mismo código que gráfico c.2.3
a <- data.frame(svytable(~conf_presi_rec + ola + zona, elsoc_diseno, round = T))
a1 <- a %>% group_by(ola, zona) %>% transmute(p_presi=Freq/sum(Freq))
b <- data.frame(svytable(~conf_part_rec + ola + zona, elsoc_diseno, round = T))
b1 <- b %>% group_by(ola, zona) %>% transmute(p_part=Freq/sum(Freq))
c <- data.frame(svytable(~conf_carb_rec + ola + zona, elsoc_diseno, round = T))
c1 <- c %>% group_by(ola, zona) %>% transmute(p_carb=Freq/sum(Freq))
datos.grafico.e.3<- cbind(a,a1,b,b1,c,c1)
...
[Data Visualization: A practical introduction (Healy, 2018)](https://socviz.co/?fbclid=IwAR2Y6epYvtWSrE9sLk5wVBJopYoh4YXKM2FkNdfMCYBuaH6hYntXM-Ep6Ec)
[Data Visualization with R (Kabacoff, 2020)](https://rkabacoff.github.io/datavis/)