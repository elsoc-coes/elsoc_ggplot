...
load(url("https://dataverse.harvard.edu/api/access/datafile/4606527")) #cargar bbdd desde url oficial       
                                                                       #alojado en dataverse
...
elsoc_long$conf_part <- sjlabelled::set_label(elsoc_long$conf_presi, label = 
...
elsoc_long$conf_carb <- sjlabelled::set_label(elsoc_long$conf_presi, label = 
...
##### c.1.1 Grafico de Barras con X = 'Categoría' en Ola = 'año'
...
##### c.1.2 Grafico de Barras con X = 'Categoría' 

Este tipo de gráficos se utiliza para visualizar respuestas desglosada por año, lo cual es útil en estudios longitudinales debido a su potencial de comparación de las respuestas señaladas por los encuestados en los distintos años de participación. 
...
##### c.1.3 Grafico de Barras con X = 'Ola' - Dodge
...
##### c.1.4 Grafico de Barras con X = 'Ola' - Stack

Este gráfico tiene como finalidad la misma interpretación que el anterior, pero su forma de visualización es distinta. Se utiliza para visualizar cada año en forma individual de forma Stack.
...
##### c.2.1 Dos categorías de agrupación: Edad y Ola

En el gráfico se puede ver la comparación etaria respecto a la confianza al presidente de la república, según año. Este gráfico es muy útil para la comparación de variables por agrupación.
...
##### c.2.2 Grafico de Barra para tres variables por Ola
...
c.2.2 <-datos.subset %>% 
  ...
  theme_bw() + 
    ...
    theme(legend.position = 'top',
        legend.title = element_blank()) +
    ggtitle('"Nada" , "Algo" y "Mucha" Confianza en instituciones por año')
c.2.2
```

##### c.2.3 Grafico de Barra para medir cambio en el tiempo de una variable
...
c.2.3 <-ggplot(datos.grafico, aes(y=porcentaje, x=cambio_estatus,fill=respuesta ,label = 
