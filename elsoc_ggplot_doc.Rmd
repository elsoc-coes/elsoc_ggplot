...
library(pacman)
pacman::p_load(car,dplyr,panelr,stringr,tidyverse,ggplot2,survey,ggrepel)
...
```{r edad_ola}
a1 <- data.frame(svytable(~conf_presi + ola + edad, elsoc_diseno, round = F))
...
```{r}
a1 <- data.frame(svytable(~conf_presi_rec + ola + edad, elsoc_diseno, round = F))
a2 <- a1 %>% group_by(edad, ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$edad <- NULL
a2$ola <- NULL
...
b <- data.frame(svytable(~conf_part_rec + ola, elsoc_diseno, round = F))
b1 <- b %>% group_by(ola) %>% transmute(p_part=Freq/sum(Freq))
c <- data.frame(svytable(~conf_carb_rec + ola, elsoc_diseno, round = F))
...
```{r}
#Paso 1
a1 <- data.frame(svytable(~idcoal + ola + idencuesta, elsoc_diseno, round = F))
...
etiquetas.d.2 <- na.omit(etiquetas.d.2)
...
etiquetas.d.3 <- na.omit(etiquetas.d.3)
...
```{r}
#Paso 1
a1 <- data.frame(svytable(~conf_presi + ola, elsoc_diseno, round = F))
a2 <- a1 %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$ola <- NULL
...
e.1 <- ggplot(datos.e.1,aes(y = porcentaje, x = ola, color = conf_presi, group = conf_presi,
    ...
    ylab(label = NULL) +
    xlab(label = NULL) +
    scale_color_viridis_d(begin = .3, end = .8, direction = 1, option = 'viridis') +
...
```{r}
#Paso 1: Mismo código que gráfico c.2.3
a <- data.frame(svytable(~conf_presi_rec + ola + zona, elsoc_diseno, round = F))
a1 <- a %>% group_by(ola, zona) %>% transmute(p_presi=Freq/sum(Freq))
b <- data.frame(svytable(~conf_part_rec + ola + zona, elsoc_diseno, round = F))
b1 <- b %>% group_by(ola, zona) %>% transmute(p_part=Freq/sum(Freq))
c <- data.frame(svytable(~conf_carb_rec + ola + zona, elsoc_diseno, round = F))
...
Healy, K. (2018). *Data Visualization: A practical introduction*. Recuperado de: [https://socviz.co/?fbclid=IwAR2Y6epYvtWSrE9sLk5wVBJopYoh4YXKM2FkNdfMCYBuaH6hYntXM-Ep6Ec](https://socviz.co/?fbclid=IwAR2Y6epYvtWSrE9sLk5wVBJopYoh4YXKM2FkNdfMCYBuaH6hYntXM-Ep6Ec)
