...
#Paso 2
b1 <- data.frame(svytable(~idcoal + ola, elsoc_diseno, round = F))
b2 <- b1 %>% 
  group_by(ola) %>% 
...
```{r}
#Paso 1
a1 <- data.frame(svytable(~idcoal + ola + idencuesta + socup + sexo, elsoc_diseno, round = F))
a1 <- a1 %>% dplyr::filter(Freq>0)
a2 <- a1 %>% group_by(ola) %>% transmute(porcentaje=Freq/sum(Freq)) 
a2$ola <- NULL
...
#Paso 2
b1 <- data.frame(svytable(~idcoal + ola + socup + sexo, elsoc_diseno, round = F))
b2 <- b1 %>% 
  group_by(ola) %>% 
  transmute(porcentaje=Freq/sum(Freq)) %>% 
  mutate(idencuesta = 1)
